...
void mandelbrot(int height,
		...
		int setup_return,
		Display * display,
		Window win, GC gc, double scale_color, double min_color)
#else
		int **output)
#endif
{
	...
	for (row = 0; row < height; ++row) {
		...
		for (col = 0; col < width; ++col) {
			...
			c.imag =
			    imag_min +
			    ((double)(height - 1 - row) * scale_imag);
...
int main(int argc, char *argv[])
{
...
#if _DISPLAY_
			fprintf(stderr,
				"Usage: %s [-i maxiter -w windowsize -c x0 y0 -s size]\n",
				argv[0]);
#else
			fprintf(stderr,
				...
				argv[0]);
			fprintf(stderr,
				"       -o to write computed image to disk (default no file generated)\n");
#endif
			fprintf(stderr,
				"       -i to specify maximum number of iterations at each point (default 1000)\n");
#if _DISPLAY_
			fprintf(stderr,
				"       -w to specify the size of the display window (default 800x800 pixels)\n");
#else
			fprintf(stderr,
	...
	#pragma omp parallel
	{
		...
		mandelbrot(height, width, real_min, imag_min, scale_real,
			   scale_imag, maxiter, setup_return, display, win, gc,
			   scale_color, min_color);
#else
		mandelbrot(height, width, real_min, imag_min, scale_real,
			   scale_imag, maxiter, output);
#endif
	...
	if (fp != NULL) {
		for (int row = 0; row < height; ++row)
			if (fwrite(output[row], sizeof(int), width, fp) !=
			    width)
				fprintf(stderr,
					"Output file not written correctly\n");
	}
#endif
